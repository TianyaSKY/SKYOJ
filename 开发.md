...
3. **异步处理**:
    * 判题是一个耗时操作。建议引入 Redis \+ Celery。Flask 接收提交后，将任务扔给 Celery Worker，Worker 跑完 Docker 后更新
      MySQL 状态。这样前端不会卡死，可以轮询状态。
...
- [x] **数据模型**：User, Problem, Submission, Dataset, Exam, SysDict 模型定义及迁移。
- [x] **API 接口**：
    - 用户认证 (`auth.py`)：注册、登录。
    - 题目管理 (`problem.py`)：增删改查、测试用例 Zip 上传与解压、**测试点打包下载/清空**。
    - 提交管理 (`submission.py`)：异步判题调度、提交详情查询、**考试有效性校验**。
    - 考试管理 (`exam.py`)：考试创建、题目绑定、密码验证。
    - 智能助手 (`llm.py`)：**集成 OpenAI SDK，支持 DeepSeek 思维链，动态配置**。
    - 系统配置 (`sys_dict.py`)：系统级常量管理。
