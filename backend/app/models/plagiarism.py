...
class PlagiarismLog(db.Model):
    ...
    submission_id = db.Column(db.Integer, db.ForeignKey('submissions.id', ondelete='CASCADE'), unique=True, nullable=False)
    ...
    # Relationships
    submission = db.relationship('Submission', foreign_keys=[submission_id], backref=db.backref('plagiarism_log', uselist=False))
